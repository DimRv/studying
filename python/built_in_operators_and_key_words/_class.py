...
4. Композиция - классы содержат в себе другие классы
...
15. Делегирование - передача выполнения метода другому объекту, например, экземпляру подкласса
...
__getattribute__ -> выполнятся при обращении ко всем атрибутам (return object.__getattribute__(self, attr))
__setattr__ - при изменении атрибута (object.__setattr__(self, name, value), self.__dict__[name] = value)
__delattr__ - при удалении атрибута
__add__ и __radd__ при участии в операции "+"
__iadd__ - +=
__sub__ и __rsub__ при участии в операции "-"
__mod__ и __rmod__ - при операциях "%"
...
__get__, __set__, __delete__ - классы содержащие это методы называются дескрипторы,такие классы присваиваются атрибуту другого
класса и вызываются при обращении
...
print("Пример4".center(40, "-"))


class Descriptor:
    """
    self - экземпляр класса Descriptor,
    instance - экземпляр клиентского класса, содержащий дескриптор
    owner - сам класс экземпляра instance
    """
